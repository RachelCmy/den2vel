...
from lib.ops import *
from lib.npops import vel_uv2hsv, jacobian2D_np, vor_rgb, applyToGrid, FFmpegTool
from lib._settings import ffmpegpath

# custom MyShape
class MyShape(object):
    def __init__(self, parent, center, radius, tex_file_path="./textures/heart.png"):
        self.parent = parent
        grid_sz = parent.getGridSize()
        self.grid_sz = [int(grid_sz.z),int(grid_sz.y),int(grid_sz.x)]
        self.center = center
        self.radius = radius * 1.6
        tex_file = cv.imread(tex_file_path)[::-1,...]
        self.tex_file = cv.cvtColor(tex_file, cv.COLOR_BGR2GRAY)
    ...
    def applyToGrid(self, grid, value, mod):
        ch = 3 if mod == "vel" else 1
        npGrid = np.zeros(self.grid_sz+[ch], np.float32)
        if mod == "vel":
            copyGridToArrayMAC( target=npGrid, source=grid )
        elif mod == "flag":
            copyGridToArrayInt( target=npGrid, source=grid )
        elif mod == "den":
            copyGridToArrayReal( target=npGrid, source=grid )
        applyToGrid(npGrid, self.center, self.radius, self.tex_file, value)
        if mod == "vel":
            copyArrayToGridMAC( target=grid, source=npGrid )
        elif mod == "flag":
            copyArrayToGridInt( target=grid, source=npGrid )
        elif mod == "den":
            copyArrayToGridReal( target=grid, source=npGrid )

    def computeLevelset(self): # expensive!
        obsphi = self.parent.create(LevelsetGrid)
        color_mask = np.zeros(self.grid_sz+[1], np.float32)
        applyToGrid(color_mask, self.center, self.radius, self.tex_file, 255)
        
        dst1 = cv.distanceTransform(color_mask[0,...,0].astype(np.uint8), cv.DIST_L2, 5)
        dst2 = cv.distanceTransform((255-color_mask[0,...,0]).astype(np.uint8), cv.DIST_L2, 5)
        copyArrayToGridReal( target=obsphi, source=dst2-dst1 )
        return obsphi
...
for obs_key in obs_random: 
    ...
    if obs_n == 1:
        ...
        shct = sl_gs*vec3(copos[0],copos[1],copos[2])
        obs = MyShape( parent=sl, center=shct, radius=res*copos[3]*0.25)
        obsL = MyShape( parent=sl, center=shct, radius=(res*copos[3]*0.25+7))
        print("MyShape", copos)
    elif obs_n == 2:
        copos = np.random.rand(4) * 0.6 + 0.2 # 0.2 - 0.8
        copos[1] = copos[1] * 0.5 + 0.3  # 0.4 - 0.7
        ...
        obs = MyShape( parent=sl, center=shct, radius=bsize.x, tex_file_path=star)
        obsL = MyShape( parent=sl, center=shct, radius=bsizeL.x, tex_file_path=star)
        print("Box", copos)
    if obs is not None:
        sl_obs += [obs]
        sl_obs_L += [obsL]
...
while t < steps + timeOffset:
    ...
    if tttt >= 0:
        ...
        for obs,obsL,obs_v in zip(sl_obs, sl_obs_L, moving): 
            ...
            if obsL.__class__.__name__ == "MyShape":
                obsL.applyToGrid(grid=obsVel, value=np.float32([obsVelVec.x,obsVelVec.y,obsVelVec.z]), mod="vel")
            else:
            ...
            # clear smoke inside
            if obs.__class__.__name__ == "MyShape":
                obs.applyToGrid(grid=sl_density, value=0, mod="den") 
                obs.applyToGrid(grid=NETdensity, value=0, mod="den") 
            else:
